...
def test_session_agent_is_resolved_from_runtime_store() -> None:
    store = Mock()
    store.load_conversation_session.return_value = Mock(agent_id="Pickle")
    ...
    store.load_conversation_session.assert_called_once_with("session-1")


def test_missing_session_stops_before_runtime_assembly() -> None:
    store = Mock()
    store.load_conversation_session.return_value = None
    application = RuntimeApplication(
