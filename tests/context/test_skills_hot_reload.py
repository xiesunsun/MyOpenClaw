"""skills_path 非空时，两次上下文构建之间的目录变化会反映到 catalog。"""
...
def test_model_context_builder_rediscovers_skills_when_skills_path_set():
    with TemporaryDirectory() as tmpdir:
        ...
        second = asyncio.run(builder.build_model_context(run=run, session=session))
        assert "alpha: First skill." in second.system.as_text()
        assert "beta: Second skill." in second.system.as_text()


def test_model_context_builder_uses_agent_skills_when_skills_path_is_none():
