...
本文记录已经逐项确认的实体结论；实施顺序见 [`Agent Runtime 重构实施计划`](./2026-08-24-agent-runtime-refactoring-plan.md)。既有 Runtime 实体已于 2026-08-25 对齐；2026-08-27 实施的 ModelCall 只解决完整模型调用数据的可靠承载，不提前引入 Eval、诊断规则、展示模型或自进化实体。
...
4. ModelCall 可靠数据底座已经实施；后续展示只从可靠事实做只读投影，不恢复 RequestSnapshot 旁路。
