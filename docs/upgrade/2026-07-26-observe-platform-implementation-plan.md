...
- [x] **Step 1: 失败测试** `tests/observe/test_model.py`：构造含 1 turn/1 step/1 tool 的 `SessionTrajectory`，断言 `json.dumps(trajectory_to_dict(t))` 不抛且往返后 `["turns"][0]["steps"][0]["usage"]["actual_input"]` 正确。
- [x] **Step 2: 跑测试确认 FAIL**（ModuleNotFoundError）。
- [x] **Step 3: 实现 model.py**（如上定义 + `trajectory_to_dict = asdict` 包装）。
- [x] **Step 4: 跑测试 PASS。**
- [x] **Step 5: Commit** `feat(observe): 轨迹值对象`
...
- [x] **Step 1: 失败测试矩阵**（复用 `tests/runs/test_turn_usage.py` 的 `_assistant` 构造手法，本地重写 helper）：
...
- [x] **Step 2: FAIL 确认。**
- [x] **Step 3: 实现 collector.py。**
- [x] **Step 4: PASS + 全量 `uv run --with pytest python -m pytest tests/ -q` 绿。**
- [x] **Step 5: Commit** `feat(observe): Session→轨迹采集器`
...
- [x] **Step 1: 失败测试**：
  ...
  - 文件不存在 → None
- [x] **Step 2: FAIL。** **Step 3: 实现。** **Step 4: PASS。**
- [x] **Step 5: Commit** `feat(observe): trace 白名单时序增强`
...
- [x] **Step 1: 失败测试**：
  ...
  - 空列表 → `ValueError`
- [x] **Step 2: FAIL。** **Step 3: 先 dataviz skill，再实现模板。** **Step 4: PASS。**
- [x] **Step 5: Commit** `feat(observe): 自包含 HTML 观测平台渲染`
...
- [x] **Step 1: 失败测试**（`typer.testing.CliRunner` + `monkeypatch.setenv("PICKEL_HOME", tmp_path)`）：预置 SQLite 会话（SQLiteSessionRepository.create + append_entries）→ `observe --out x.html` 退出码 0、文件存在、内容含 session_id；空库 → 退出码 1。
- [x] **Step 2: FAIL。** **Step 3: 实现。** **Step 4: PASS + 全量绿。**
- [x] **Step 5: Commit** `feat(cli): pickel observe 导出观测平台`

### Task 6: 变异实测 + 真机验证

- [x] 变异 1：collector 的 tool 配对改按序配对 → `test_collector` 必须变红；还原。
- [x] 变异 2：trace_reader 白名单放开读 `user_text` 塞进 marker → 红线测试必须变红；还原。
- [x] 真机：`uv run pickel observe --out /tmp/.../observe.html` 用 ~/.pickel 真实数据生成并抽查内容。
- [x] 全量 `uv run --with pytest python -m pytest tests/ -q` 绿；Commit（若有修正）。
