...
from myopenclaw.application import RuntimeEvent, RuntimeEventType, ToolResultView
from myopenclaw.application.contracts import ModelToolCallBatch, TurnResult
...
class ChatEventRenderer:
    ...
    async def handle_event(self, event: RuntimeEvent) -> None:
        if event.event_type == RuntimeEventType.MODEL_STEP_STARTED:
            self._render_message("Thinking", Text(f"Step {event.step_index}"), style="magenta")
            return
        ...
        if event.event_type in {
            ...
            tool_result = event.tool_result or ToolResultView(content="")
            self._render_message(
                "Tool",
        ...
        if event.event_type == RuntimeEventType.ASSISTANT_MESSAGE:
            self._render_turn_result(
                TurnResult(text=event.text, metadata=event.metadata),
            )

    @classmethod
    def render_tool_batch_transcript(cls, batch: ModelToolCallBatch) -> list[tuple[str, Text]]:
    ...
    def render_turn_output(self, turn_result: TurnResult) -> None:
        for batch in turn_result.tool_batches:
            for style, renderable in self.render_tool_batch_transcript(batch):
                self._render_message("Tool", renderable, style=style)
        self._render_turn_result(turn_result)

    def _render_turn_result(self, turn_result: TurnResult) -> None:
        content: RenderableType = Markdown(turn_result.text)
        if turn_result.metadata is not None:
            content = Group(
                Markdown(turn_result.text),
                self._render_assistant_footer(turn_result.metadata),
            )
        self._render_message("Assistant", content, style="yellow")

    @classmethod
    def _render_tool_started(cls, name: str, arguments: dict[str, object]) -> Text:
        return Text(f"{cls._format_tool_label(name, arguments)}\nstatus: running")
    ...
    def _render_tool_finished(
        ...
        lines = [cls._format_tool_label(name, arguments), f"status: {status}"]
    ...
    def _render_assistant_footer(self, metadata) -> Text:
