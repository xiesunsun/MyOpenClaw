...
from myopenclaw.application.ports import LLMPort
from myopenclaw.infrastructure.config.model_config import ModelConfig
...
class GeminiAdapter(LLMPort):
    ...
    @classmethod
    def from_config(cls, config: ModelConfig) -> "GeminiAdapter":
        return cls(
            model=config.model,
            ...
            types.Tool(
                function_declarations=[
                    types.FunctionDeclaration(
                        **GeminiAdapter._build_function_declaration(tool_spec)
                    )
                ]
            )
    ...
    @staticmethod
    def _build_contents(messages: list[ModelMessage]) -> list[types.Content]:
        ...
        for message in messages:
            ...
            if message.tool_call_batch is not None:
                contents.extend(GeminiAdapter._build_batch_contents(message))
                continue
    ...
    @staticmethod
    def _build_batch_contents(message: ModelMessage) -> list[types.Content]:
        ...
        response_parts = [
            types.Part(
                function_response=types.FunctionResponse(
                    id=tool_call.id,
                    name=tool_call.name,
                    response=GeminiAdapter._build_function_response_payload(tool_result),
                )
            )
            for tool_call, tool_result in GeminiAdapter._ordered_batch_pairs(batch)
        ]
    ...
    ) -> list[tuple[ModelToolCall, ModelToolResult]]:
        results_by_call_id = {
    ...
    @staticmethod
    def _build_function_response_payload(tool_result: ModelToolResult) -> dict[str, Any]:
        response = (
            {"error": tool_result.content}
            if tool_result.is_error
            else {"output": tool_result.content}
        )
    ...
    @staticmethod
    def _extract_provider_finish_reason(
        response: types.GenerateContentResponse,
    ) -> str | None:
        candidate = GeminiAdapter._primary_candidate(response)
