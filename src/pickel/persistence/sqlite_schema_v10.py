"""SQLite v10 领域 schema。

该模块只负责建立目标 schema，不承载 v9 兼容或 Runtime 存储读写逻辑。
所有可由 SQLite 表达的跨实体约束都放在 DDL 中；最终 Assistant Node 与
Operation 所属 Session 的关系由两个触发器补足（该关系无法用当前列集合
直接写成外键）。
"""
...
# 便于调用方按语义命名，同时保持只有一份 SQL 来源。
SCHEMA_V10_SQL = SCHEMA_SQL


def create_schema(connection: sqlite3.Connection) -> None:
    """在连接上创建 SQLite v10 schema。

    该包装只接受空库或已经是 v10 的连接。对象创建和 user_version 设置在
    一个事务中完成；v9 或其他版本必须先经过显式迁移，不能在这里隐式保留
    双轨表。
    """
...
def create_schema_objects(connection: sqlite3.Connection) -> None:
    """在调用方当前事务内逐条创建 v10 表、索引和触发器。

    该函数不执行 ``BEGIN``、``COMMIT`` 或 ``PRAGMA user_version``，适用于
    v9→v10 迁移事务。使用 ``sqlite3.complete_statement`` 拆分 trigger 的
    ``BEGIN … END``，避免 ``executescript`` 隐式提交调用方事务。
    """
    ...
    if any(line.strip() for line in statement_lines):
        raise ValueError("SQLite v10 schema SQL 包含未结束的语句")
