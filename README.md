...
Runtime 重构的当前合同只有下面六份文档。涉及实体、方法、字段、持久化、恢复或 Runtime 边界时，先按「命名 → 实体决策 → 领域合同 → 实施计划」阅读；其他文档不能覆盖这些合同。
...
| [数据库实体设计](docs/upgrade/2026-07-12-db-entities.md) | SQLite v11 表、字段、约束、事务和迁移 |
...
除上面六份当前合同外，`docs/upgrade/` 中的设计稿和实施计划、`docs/superpowers/` 中的规格/计划，以及 `docs/openviking/` 和 [排障手册](docs/troubleshooting.md) 都是历史记录或独立领域参考。它们不会改变当前 Runtime 的实体和边界。
